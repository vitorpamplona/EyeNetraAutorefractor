...
public class PrescriptionTest {
    ...
    public void testCopyConstructor() {
        EventHistory placeHolder = new EventHistory();
        ComputedPrescription placeHolder2 = new ComputedPrescription();

        Prescription p = new Prescription(1f, 2f, 3f, RefractionType.NETRA);
        p.setAddLens(4f);
        p.setId(UUID.randomUUID());
        p.setCyclo(true);
        p.setHistory(placeHolder);
        p.setVaCorrected(0.76f);
        p.setOriginalData(placeHolder2);
        ...
        assertEquals(p.getFittingQuality(), pCopy.getFittingQuality());
        assertEquals(p.getOriginalData(), pCopy.getOriginalData());
        assertEquals(p.getId(), pCopy.getId());
        assertEquals(p.getProcedure(), pCopy.getProcedure());
        assertEquals(p.getHistory(), pCopy.getHistory());
    }
    ...
    public void testCopyCylModes() {
        ...
        assertEquals(0, pNeg.getAxis(), 0.01);
        ...
        assertEquals(0, pNeg2.getAxis(), 0.01);
