 ...
 */
public class AngleChange360Dispatcher extends NumberChangeDispatcher {

    public AngleChange360Dispatcher(double stepSize) {
        super(stepSize);
    }

    public double difference() {
        return Math.abs(AngleDiff.diff360((float) value, (float) lastDispatchedValue));
    }
    ...
    public void refreshListeners() {
        for (NumberChangedListener l : listeners) {
            ...
            if (Math.abs(value - lastDispatchedValue) > 180) {
                diff = -diff;
            }
