...
public class ExamResults implements Serializable {
    ...
    public float getInterpupillaryDistanceMM() {
        if (getLeftEye().getNetra().getNosePupilDistance() != null && getRightEye().getNetra().getNosePupilDistance() != null) {
            return getLeftEye().getNetra().getNosePupilDistance() + getRightEye().getNetra().getNosePupilDistance();
        }
        return 0;
    }
    ...
    public void addTestResult(int testingEye, MeridianPower result) {
        switch (testingEye) {
            case RIGHT:
                rightEye().add(result);
                break;
    ...
    public ComputedPrescription newTestResult(int testingEye) {
        switch (testingEye) {
            case RIGHT:
                return rightEye().getNetra().getOriginalData();
            case LEFT:
                return leftEye().getNetra().getOriginalData();
        }
        // error.
        return null;
    }

    public EventHistory newEventHistory(int testingEye) {
        switch (testingEye) {
            case RIGHT:
                return rightEye().getNetra().history();
            case LEFT:
                return leftEye().getNetra().history();
        }
        // error.
        return null;
    }
